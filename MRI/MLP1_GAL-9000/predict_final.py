# GAL-9000 group members Alex Button, Leon Bichmann and Gisela Gabernet
# ETH Zuerich | October 2016
...
class CenterCutCubes(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X_train, y=None):
        # selecting the x, y, z dimensions of the prism to extract (maximum variance zone)
        cut = []
        for n in X_train:
            cut.append(n[self.x1:self.x2, self.y1:self.y2, self.z1:self.z2])
        self.cut = cut
        descriptor = []
        # Performing coarse-graining by taking the maximum intensity value in cubes of defined dimensions (E.g. 3x3x3).
        for n in cut:
            int_cubes = []
...
pipe = Pipeline([('cut', CenterCutCubes(size_cubes=3)), # Extracting prism of maximum variance from 3D brain images and coarse-graining it by taking the maximum intensity of a voxel in a 3x3x3 voxel cube.
...
with open("SubmissionFinal.csv", mode='w') as f:
    ...
    for idx, pred in enumerate(predictions):
        pred = round(pred, 0) # Rounding values for predictions.
